...
(function () {
  ...
  const EMOJI_DATA = {
    ...
    '工具': ['🛠️','🔧','🔨','🪛','🪚','⚙️','🧰','🪜','🔩','📐','📏','✂️','🖇️','📎','📌','📍','🔍','🔎','🔑','🗝️','🔒','🔓','🧲','🔗','⛓️','🪝','📡','🛰️','💾','🗄️'],
    '安全': ['🔒','🔓','🔏','🔐','🔑','🗝️','🛡️','⚠️','🚫','⛔','📵','🔞','🚷','🚯','🚳','🚱','🚭','🆘','🔰','🛂','🛃','🛄','🛅','👮','🕵️','🚨','🚔','🛡️','🪪','🦺'],
    '系统': ['⚙️','🛠️','🔧','📊','📈','📉','💾','🗄️','🗂️','📂','📁','🖥️','💻','🖨️','⌨️','🖱️','🔌','🔋','📡','🌐','🛰️','🧰','🧪','🧬','📡','♻️','🔄','🔁','🔂','⏰'],
    '动物': ['🐶','🐱','🐭','🐹','🐰','🦊','🐻','🐼','🐨','🐯','🦁','🐮','🐷','🐸','🐵','🐔','🐧','🦅','🦉','🦄','🐝','🐞','🦋','🐢','🐍','🐙','🐠','🐳','🐬','🦈'],
    '自然': ['☀️','🌙','⭐','🌟','✨','⚡','🌈','☁️','🌧️','⛈️','❄️','☃️','🔥','💧','🌊','🌍','🌎','🌏','🌐','🌋','🏔️','🌲','🌳','🌴','🌵','🌸','🌹','🌻','🍀','🪴'],
    '符号': ['✨','💫','⭐','🌟','🔥','⚡','💥','💯','✅','☑️','✔️','❌','❎','➕','➖','➗','✖️','♾️','‼️','⁉️','❓','❗','💢','🔔','🔕','🆕','🆓','🆗','🆙','🆒','🔝','🔚','🔙','🔛','🔜','©️','®️','™️','♻️','🚫'],
    '颜色': ['🔴','🟠','🟡','🟢','🔵','🟣','🟤','⚫','⚪','🟥','🟧','🟨','🟩','🟦','🟪','🟫','⬛','⬜','🔶','🔷','🔸','🔹','🔺','🔻','💠','🔘','🔳','🔲'],
    '旗帜': ['🏁','🚩','🎌','🏴','🏳️','🏳️‍🌈','🏴‍☠️','🇨🇳','🇭🇰','🇲🇴','🇹🇼','🇺🇸','🇯🇵','🇰🇷','🇬🇧','🇫🇷','🇩🇪','🇷🇺','🇨🇦','🇦🇺','🇸🇬','🇪🇺','🇺🇳'],
  };
  ...
  function renderSmallIcon(iconStr, url) {
    ...
    var src = '';
    var imgExtRe = /\.(png|svg|ico|jpg|jpeg|webp|gif)$/i;
    if (/^url:/i.test(iconStr)) {
      var raw = iconStr.replace(/^url:/i, '').trim();
      src = /^https?:\/\//i.test(raw) ? raw : 'http://' + raw;
    } else if (/^https?:\/\//i.test(iconStr)) {
      src = iconStr;
    } else if (iconStr && !iconStr.includes('/') && imgExtRe.test(iconStr)) {
      // 本地文件（带后缀，无路径）
      src = 'ico/' + iconStr;
    } else if (iconStr && iconStr.includes('/')) {
      // 含路径，视为远程地址
      src = /^https?:\/\//i.test(iconStr) ? iconStr : 'http://' + iconStr;
    } else if (iconStr) {
      // 本地名（无后缀）
      src = 'ico/' + iconStr + '.png';
    } else {
  ...
  // ==================== 本地图标选择器 ====================
  var _localIconsCache = null;

  function loadLocalIcons() {
    if (_localIconsCache) return Promise.resolve(_localIconsCache);
    // 利用 nginx autoindex JSON 列出 ico/ 目录
    return fetch('ico/', { cache: 'no-cache', headers: { 'Accept': 'application/json' } })
      .then(function (r) {
        if (!r.ok) return [];
      ...
      .then(function (list) {
        if (!Array.isArray(list)) { _localIconsCache = []; return _localIconsCache; }
        _localIconsCache = list
          .filter(function (it) { return it && it.type === 'file' && /\.(png|svg|ico|jpg|jpeg|webp|gif)$/i.test(it.name); })
          .map(function (it) { return it.name; })
          .sort();
        return _localIconsCache;
      })
      .catch(function () { _localIconsCache = []; return _localIconsCache; });
  }

  function renderLocalIconPicker() {
    var container = $('#local-icon-picker');
    if (!container) return;
    loadLocalIcons().then(function (icons) {
      if (!icons.length) {
        container.innerHTML = '<div class="local-icon-empty">本地图标请放到 <code>ico/</code> 目录，可直接输入图标名（无后缀会匹配 <code>png→svg→ico→jpg</code>）</div>';
        return;
      }
      var current = ($('#item-icon-url').value || '').trim();
      var html = '<div class="form-label" style="margin-top:0.5rem">本地图标（点击选用）</div><div class="local-icon-grid">';
      icons.forEach(function (name) {
        var sel = name === current ? ' selected' : '';
        html += '<button type="button" class="local-icon-item' + sel + '" data-name="' + escapeHtml(name) + '" title="' + escapeHtml(name) + '">';
        html += '<img src="ico/' + escapeHtml(name) + '" alt="" loading="lazy">';
      ...
      $$('.local-icon-item', container).forEach(function (btn) {
        btn.addEventListener('click', function () {
          var name = this.dataset.name;
          $('#item-icon-url').value = name;
          $$('.local-icon-item', container).forEach(function (b) { b.classList.remove('selected'); });
